...
public interface DietRepository extends CrudRepository<Diet, Integer>{
	...
	@Query("select d from Diet d where user.id = :userId and d.id in (select tp.diet.id from TrainingPlanDiet tp where tp.trainingPlan.id = :trainingPlanId)")
	List<Diet> findAssignedToTrainingPlan(@Param("trainingPlanId") Integer trainingPlanId, @Param("userId") Integer userId);
